...
---@class LargeCogwheel : KineticBlock
local LargeCogwheel = setmetatable({}, {__index = KineticBlock});
LargeCogwheel.__index = LargeCogwheel;

LargeCogwheel.is_cogwheel = true;
LargeCogwheel.is_large_cogwheel = true;

function LargeCogwheel:on_placed()
    KineticBlock.on_placed(self);
    local ent = entities.spawn(make_entity_prefix("large_cogwheel"), make_position_offs(self.x, self.y, self.z, 0.5));
...
function LargeCogwheel:get_rotation_offset()
    local sum = math.floor(self.x + self.y + self.z)
    return ((sum + 1) % 2) * 11.25
end

function LargeCogwheel:get_bone_matrix()
...
function LargeCogwheel:on_render(gtime, dt)
...
function LargeCogwheel:get_connections()
    ...
    for _, p in ipairs(get_diag_offsets(axis)) do
        ...
        if neighbor and neighbor.is_cogwheel and not neighbor.is_large_cogwheel then
            table.insert(conn, {x = nx, y = ny, z = nz, dir_mult = -1, ratio_mult = 2.0});
        end
    end
...
function LargeCogwheel:can_connect_from(x, y, z)
    ...
    if src.get_axis_name and src:get_axis_name() == self:get_axis_name() then
        if dist_sqr == 2 and src.is_cogwheel and not src.is_large_cogwheel then
            return true;
        end
    end
